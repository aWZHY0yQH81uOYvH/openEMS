...
__device__ void UpdateCurrents(Engine_CUDA *instance, unsigned int x, unsigned int y, unsigned int z)
{
	CUDA_VECTOR i = instance->curr[x][y][z];
	CUDA_VECTOR v = instance->volt[x][y][z];
	CUDA_VECTOR vx = instance->volt[x+1][y][z];
	CUDA_VECTOR vy = instance->volt[x][y+1][z];
	CUDA_VECTOR vz = instance->volt[z][y][z+1];
	CUDA_VECTOR iv = instance->Op->iv[x][y][z];
	CUDA_VECTOR ii = instance->Op->ii[x][y][z];

	i.x = i.x * ii.x + iv.x * (v.z - vy.z - v.y + vz.y);
	i.y = i.y * ii.y + iv.y * (v.x - vz.x - v.z + vx.z);
	i.z = i.z * ii.z + iv.z * (v.y - vx.y - v.x + vy.x);

	instance->curr[x][y][z] = i;
}
...
__global__ void VoltageKernel(CUDA_VECTOR ***volt, CUDA_VECTOR ***curr, CUDA_VECTOR ***opvi, CUDA_VECTOR ***opvv, unsigned int numLines[]) {
	const unsigned int pos[] = {
		blockDim.x * blockIdx.x + threadIdx.x,
		blockDim.y * blockIdx.y + threadIdx.y,
		blockDim.z * blockIdx.z + threadIdx.z
...
__global__ void CurrentKernel(Engine_CUDA *instance, unsigned int numLines[]) {
	const unsigned int pos[] = {
		blockDim.x * blockIdx.x + threadIdx.x,
		blockDim.y * blockIdx.y + threadIdx.y,
		blockDim.z * blockIdx.z + threadIdx.z
	...
	if(pos[0] >= numLines[0]-1 || pos[1] >= numLines[1]-1 || pos[2] >= numLines[2]-1) return;

	UpdateCurrents(instance, pos[0], pos[1], pos[2]);
}
...
bool Engine_CUDA::IterateTS(unsigned int iterTS)
{
	...
	*/
	...
	{
		VoltageKernel<<<m_gridDim, m_blockDim>>>(volt, curr, Op->vi, Op->vv, numLines);
		cudaError_t err = cudaGetLastError();
		if(err) throw std::runtime_error("CUDA kernel launch failure: " + std::string(cudaGetErrorString(err)));
		//CurrentKernel<<<m_gridDim, m_blockDim>>>(this, numLines);
